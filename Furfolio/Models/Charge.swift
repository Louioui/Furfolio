...
final class Charge: Identifiable {
    ...
    var behavioralTags: [String] // New property for tracking behavioral tags (e.g., Sensitive Skin, Likes/Dislikes Certain Treatments)
    ...
    // MARK: - Initializer
    init(date: Date, type: ServiceType, amount: Double, dogOwner: DogOwner, notes: String? = nil, petBadges: [String] = [], behavioralTags: [String] = []) {
        self.id = UUID()
        self.date = date
        self.type = type
        ...
        self.behavioralTags = behavioralTags // Initialize behavioral tags
    }
    ...
    func applyDiscount(_ percentage: Double) {
        guard percentage > 0 && percentage <= 100 else {
            print("Invalid discount percentage") // Informative log for invalid input
            return
        }
    ...
    func analyzeBehavior() -> String {
        ...
        // Behavioral tags are considered here
        if behavioralTags.contains("Sensitive Skin") {
            return NSLocalizedString("Behavioral analysis: Pet has sensitive skin, use appropriate products.", comment: "Behavioral analysis result")
        } else if behavioralTags.contains("Likes Certain Treatments") {
            return NSLocalizedString("Behavioral analysis: Pet prefers certain grooming treatments.", comment: "Behavioral analysis result")
        }
    ...
    static func chargesInDateRange(_ range: ClosedRange<Date>, from charges: [Charge]) -> [Charge] {
        return charges.filter { range.contains($0.date) }
    }

    static func totalRevenue(for range: ClosedRange<Date>, from charges: [Charge]) -> Double {
        return chargesInDateRange(range, from: charges)
            .reduce(0) { $0 + $1.amount }
    }
    ...
    static func averageCharge(for type: ServiceType, from charges: [Charge]) -> Double {
        ...
        return chargesOfType.reduce(0) { $0 + $1.amount } / Double(chargesOfType.count)
    }

    // MARK: - New Methods
    static func chargesForOwnerGroupedByMonth(_ owner: DogOwner, from charges: [Charge]) -> [Int: [Charge]] {
        return chargesForOwner(owner, from: charges).reduce(into: [Int: [Charge]]()) { grouped, charge in
    ...
    static func totalRevenueGroupedByMonth(for owner: DogOwner, from charges: [Charge]) -> [Int: Double] {
        return chargesForOwnerGroupedByMonth(owner, from: charges).reduce(into: [Int: Double]()) { totals, chargeGroup in
